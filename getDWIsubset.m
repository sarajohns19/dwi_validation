function [adcTable, mean_adcTable, std_adcTable, mean_SvTable, dwiTable] = getDWIsubset(dwiData_file, parameters, b_keep, TE_keep, slice)
    ...
    for d = 1:length(dwiData) % for each image in the dwiData subset
        ...
        if strcmp(dwiData(d).SequenceType, 'SOS')
            b = [10, 50, 100, 840];
            if any(matches(split(dwiData(d).Description,'_'), 'MELV4'))
                b = [10, 1100]; 
                b_keep = [1,2];
            end 
            if strcmp(dwiData(d).Filename,'REP2_SOS_MELV2_d10_b400_rot270.mat')
                b = [10, 50, 400, 800]; 
            end 
        else 
            b = [0, 50, 100, 800];
